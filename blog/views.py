...
from .models import Post, Carousel, Comment, Repository, Catalogue, User,UReport,Maincategory, Collection,Secret, RecommendList, Tag
    ...
    def get(self, request, *args, **kwargs):
        ...
        if ur.is_authenticated():
            ...
            if obj: 
                ...
                # print(pkey)
                return HttpResponse(json.dumps({"已推荐":'-1'}))
            else:
                ...
                return HttpResponse(json.dumps({"count":post.recommend_count}))
        else:
            return HttpResponse(json.dumps({"登录":'请先登录！'}))
#*********
...
class SearchView(BaseMixin, ListView):
    ...
    def get_queryset(self):
            ...
            if new_k in ['0','10','30','50']:
                global pageviews
                pageviews = new_k
            if new_k in ['all', 'twoweek', 'onemonth', 'oneyear']:
                global day
                if new_k == 'all':
                    day = 100000
                elif new_k == 'twoweek':
            ...
            if new_k in ['allblog', 'recommend_count=20', 'recommend_count=50', 'recommend_count=100']:
                global recommend
                if new_k == 'allblog':
                    recommend = 0
                elif new_k == 'recommend_count=20':
                    recommend = 20
                elif new_k == 'recommend_count=50':
                    recommend = 50
                elif new_k == 'recommend_count=100':
                    recommend = 100
            # 获取关键字
            ...
            if keyword == None:
                ...
                if (pageviews in ['0','10','30','50']) and (day in [100000, 14, 30, 365]) \
                and (recommend in [0, 20, 50, 100]):
                    default_view_count = pageviews
                    back_time = datetime.datetime.now() - datetime.timedelta(days=day)
                    recommends = recommend
                    post_list = Post.objects.filter(status=1, stype =0, ban = 0,\
                    title__contains=global_keyword, recommend_count__gte=recommends, view_count__gte=default_view_count, publish_time__gte=back_time).order_by('-publish_time')
                elif pageviews in ['0','10','30','50']:
                    default_view_count = pageviews
                    post_list = Post.objects.filter(status=1, stype =0, ban = 0,\
                    title__contains=global_keyword, view_count__gte=default_view_count).order_by('-publish_time')
                elif day in [100000, 14, 30, 365]:
                    back_time = datetime.datetime.now() - datetime.timedelta(days=day)
                    post_list = Post.objects.filter(status=1, stype =0, ban = 0,\
                    title__contains=global_keyword, publish_time__gte=back_time).order_by('-publish_time')
                elif recommend in [0, 20, 50, 100]:
                    recommends = recommend
                    post_list = Post.objects.filter(status=1, stype =0, ban = 0,\
                    title__contains=global_keyword, recommend_count__gte=recommends).order_by('-publish_time')
                
            else:
                global global_keyword
                global_keyword = keyword
                post_list = Post.objects.filter(status=1, stype =0, ban = 0,\
                    title__contains=global_keyword).order_by('-publish_time')
            return post_list
...
class TagsView(BaseMixin, ListView):
    ...
    queryset = Tag.objects.cloud_for_model(model=Post)

    def get_context_data(self, **kwargs):
        print(self.queryset)
        page = self.kwargs.get('page') or self.request.GET.get('page') or 1
        objects, page_range = paginator_tool(pages=page, queryset=self.queryset, display_amount=100)
